...
pub struct ChatServer {
    pub clients: HashMap<String, WrapperSender>
}

impl ChatServer {
    pub fn add_client(&mut self, client: WrapperSender) {
    ...
    pub fn send_message(&mut self, message_from_sender: OwnedMessage) {
        ...
        let incoming_message_details: MessageDetails = serde_json::from_str(&mfs).unwrap(); //<-- deserialize

        let receiver_username = incoming_message_details.receiver_username;

        match self.clients.get_mut(&receiver_username) {
            Some(receiver) => {
                let outgoing_message_details = MessageDetails { sender_username: String::from(""), receiver_username: String::from(""), message: incoming_message_details.message };
